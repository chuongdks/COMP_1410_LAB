...
void ShoppingBagCombinations(Item* currBag,        // Bag contents
    ...
    else // Recursive case: move one item to bag
    {                                 
        for (i = 0; i < TOTAL_ITEMS; ++i) 
        { 
            if (!beenAdded[i]) //If the Flag to check if item has been added is not true
            {
                // Move item to bag
                beenAdded[i] = true;
                currBag[bagCnt] = remainingItems[i];

                ShoppingBagCombinations(currBag, remainingItems, beenAdded, bagCnt + 1);
...
int main(void) {
    ...
    ShoppingBagCombinations(shoppingBag, possibleItems, itemBeenAdded, 0);
